...
try {
    ...
    if (!$club_id || !$game_id) {
        $check_stmt = $pdo->prepare("SELECT game_id FROM team_game_results WHERE result_id = ?");
    ...
    $stmt = $pdo->prepare("DELETE FROM team_game_results WHERE result_id = ?");
    $stmt->execute([$result_id]);

    $pdo->commit();
    $_SESSION['success_message'] = "Team game result deleted successfully!";
} catch (PDOException $e) {
    if ($pdo->inTransaction()) {
        $pdo->rollBack();
    }
    $_SESSION['error'] = "Error deleting team result: " . $e->getMessage();
}
