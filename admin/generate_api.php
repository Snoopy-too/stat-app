...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Clean up old files for this club (both .json and .php)
        $old_json = glob("../api/club_{$club_id}_*.json");
        $old_php = glob("../api/club_{$club_id}_*.php");
        $old_files = array_merge($old_json ?: [], $old_php ?: []);
        ...
        $filename = "club_{$club_id}_{$random_string}.php";
        $filepath = "../api/" . $filename;

        // Generate the PHP file content (dynamic API that queries DB each time)
        $php_content = <<<'PHPCODE'
<?php
...
PHPCODE;
        ...
        // Replace the placeholder with actual club_id
        $php_content = str_replace('CLUB_ID_PLACEHOLDER', $club_id, $php_content);
